...
def PPHMMSignatureTable_Constructor (
	...
	for SeqIDList, TranslTable in zip(SeqIDLists, TranslTableList):
		...
		if len(GenBankSeq) >= SeqLength_Cutoff: 
			...
			p = subprocess.Popen(f"hmmscan --cpu {HMMER_N_CPUs} --noali --nobias --domtblout {PPHMMScanOutFile} {HMMER_PPHMMDB} {PPHMMQueryFile}", stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell = True)
