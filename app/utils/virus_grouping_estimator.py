...
def AntiTheilsUScore(Distance_Cutoff, linkageMat, TaxoGroupingList, symmetrical=True):
    VirusGroupingList = fcluster(
        Z=linkageMat, t=Distance_Cutoff, criterion='distance')
...
def VirusGrouping_Estimator(DistMat, Dendrogram_LinkageMethod, TaxoGroupingList):
    ...
    OptimizeResult = minimize_scalar(AntiTheilsUScore, bounds=[
                                     0, 1], method='bounded', args=(linkageMat, TaxoGroupingList, True))
    OptDistance_Cutoff = OptimizeResult.x
    Score = -1*OptimizeResult.fun
    VirusGroupingList = np.array(
        fcluster(Z=linkageMat, t=OptDistance_Cutoff, criterion='distance'))
