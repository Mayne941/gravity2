...
class UcfVirusAnnotator:
    ...
    def annotate(self) -> None:
        ...
        for ShelveDir_RefVirus in self.ShelveDirs_RefVirus:
            ...
            print(
                f"Annotate unclassified viruses using the PPHMM and GOM databases of the reference viruses: {RefVirusGroup} ({RefVirusGroup_i}/{N_RefVirusGroups})")

            '''Get fpaths'''
            HMMERDir_RefVirus, HMMER_PPHMMDB_RefVirus = self.mkdirs(
                ShelveDir_RefVirus)
            ...
            HMMER_hmmscanDir_RefVirus = f"{HMMERDir_RefVirus}/hmmscan_"+''.join(
            ...
            _ = subprocess.call(
                f"rm -rf {HMMER_hmmscanDir_RefVirus}", shell=True)
        ...
        all_ucf_genomes["PPHMMSignatureTable_Dict_coo"] = {RefVirusGroup: coo_matrix(
            PPHMMSignatureTable) for RefVirusGroup, PPHMMSignatureTable in PPHMMSignatureTable_Dict.items()}
