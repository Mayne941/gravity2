...
class VirusClassificationAndEvaluation:
	...
	def classify(self):
		...
		for ShelveDir_RefVirus, GenomeSeqFile_RefVirus in zip(self.ShelveDirs_RefVirus, self.GenomeSeqFiles_RefVirus):
			...
			if self.Heatmap_WithDendrogram == True:
				'''(OPT) Draw hm/dendro'''
				heatmap_with_dendrogram(self.Bootstrap)
	...
	def main(self):
		...
		# ucf_genomes
		#"SeqIDLists", "VirusNameList", "TranslTableList",
		# ucf_annots
		# "PPHMMSignatureTable_Dict", "PPHMMLocationTable_Dict", "PPHMMSignatureTable_Dict_coo", "PPHMMLocationTable_Dict_coo", "GOMSignatureTable_Dict",
		...
		if self.UseUcfVirusPPHMMs == True:
			...
			GenomeSeqFiles_RefVirus = [None]*len(self.ShelveDirs_RefVirus)
