...
class VirusClassificationAndEvaluation:
	def __init__(self,
		...
		self.final_results["self.MaxSimScoreTable"], self.final_results["self.TaxoOfMaxSimScoreTable"], self.final_results["self.TaxoAssignmentTable"], \
			self.final_results["self.PhyloStatTable"] = np.zeros((1,0)), np.zeros((1,0)), np.zeros((1,0)), np.zeros((1,0))
	...
	def update_globals(self):
		...
		self.final_results["self.MaxSimScoreTable"], self.final_results["self.TaxoOfMaxSimScoreTable"], self.final_results["self.TaxoAssignmentTable"], \
			self.final_results["self.PhyloStatTable"] = np.zeros((self.N_UcfViruses, 0)), np.zeros((self.N_UcfViruses, 0)), np.zeros((self.N_UcfViruses, 0)), np.zeros((self.N_UcfViruses, 0))
	...
	def classify(self):
		...
		for ShelveDir_RefVirus, GenomeSeqFile_RefVirus in zip(self.ShelveDirs_RefVirus, self.GenomeSeqFiles_RefVirus):
			...
			self.final_results["MaxSimScoreTable"]		= np.column_stack((self.final_results["self.MaxSimScoreTable"], MaxSimScoreList))
			self.final_results["TaxoOfMaxSimScoreTable"]= np.column_stack((self.final_results["self.TaxoOfMaxSimScoreTable"], TaxoOfMaxSimScoreList))
			self.final_results["TaxoAssignmentTable"]	= np.column_stack((self.final_results["self.TaxoAssignmentTable"], TaxoAssignmentList))
			self.final_results["PhyloStatTable"]		= np.column_stack((self.final_results["self.PhyloStatTable"], PhyloStatList))
	...
	def group(self):
		...
		if VirusGrouping == True:
			...
			for UcfVirus_i in range(self.N_UcfViruses):
				MaxSimScore_i		= np.argmax(self.final_results["self.MaxSimScoreTable"][UcfVirus_i])
				FinalisedTaxoAssignment	= self.final_results["self.TaxoAssignmentTable"][UcfVirus_i][MaxSimScore_i]
				RefVirusGroup		= self.ShelveDirs_RefVirus[MaxSimScore_i].split("/")[-1]
				...
				if FinalisedTaxoAssignment != "Unclassified":
					FinalisedTaxoAssignmentList.append(f"{FinalisedTaxoAssignment} ({RefVirusGroup})")
					FinalisedVirusGroupingList.append(f"{FinalisedTaxoAssignment} ({RefVirusGroup})")
				else:
					if self.final_results["self.MaxSimScoreTable"][UcfVirus_i][MaxSimScore_i] <= self.DatabaseAssignmentSimilarityScore_Cutoff:
						FinalisedTaxoAssignmentList.append("Unclassified (NA)")
						FinalisedVirusGroupingList.append("Unclassified (NA)")
					else:
						FinalisedTaxoAssignmentList.append(f"Unclassified ({RefVirusGroup})")
						VirusGrouping = self.VirusGroupingDict[RefVirusGroup]["VirusGroupingList"][-self.N_UcfViruses:][UcfVirus_i]
						if VirusGrouping not in FinalisedVirusGroupingDict[RefVirusGroup]:
							# RM < NUMBER NEEDS TO BE 4 POINT PREC. TEST.
							FinalisedVirusGroupingDict[RefVirusGroup][VirusGrouping] = f"UTU{FinalisedVirusGroupingIndexDict[RefVirusGroup]}" 
							FinalisedVirusGroupingIndexDict[RefVirusGroup] += 1
						
						FinalisedVirusGroupingList.append(f"{FinalisedVirusGroupingDict[RefVirusGroup][VirusGrouping]} ({RefVirusGroup})")
		...
		RemainedUcfVirus_IndexList 		= np.where(list(map(all, self.final_results["self.MaxSimScoreTable"] <= self.DatabaseAssignmentSimilarityScore_Cutoff)))[0]
		...
		if self.Bootstrap == True:
			...
			for UcfVirus_i in range(self.N_UcfViruses):
				...
				for Bootstrap_i in range(0, self.N_Bootstrap):
					MaxSimScore_i		= np.argmax(self.MaxSimScoreDistTable[Bootstrap_i][UcfVirus_i])
					FinalisedTaxoAssignment	= self.TaxoAssignmentDistTable[Bootstrap_i][UcfVirus_i][MaxSimScore_i]
					if FinalisedTaxoAssignment != "Unclassified":
						BootstrappedFinalisedTaxoAssignmentList.append(f"{FinalisedTaxoAssignment} ({self.ShelveDirs_RefVirus[MaxSimScore_i].split('/')[-1]})")
					else:
						if self.MaxSimScoreDistTable[Bootstrap_i][UcfVirus_i][MaxSimScore_i] <= self.DatabaseAssignmentSimilarityScore_Cutoff:
							...
							BootstrappedFinalisedTaxoAssignmentList.append(f"Unclassified ({self.ShelveDirs_RefVirus[MaxSimScore_i].split('/')[-1]})")
	...
	def write(self, ClassificationResultFile):
		...
		if self.Bootstrap == True:
			np.savetxt(	fname = ClassificationResultFile,
					X = np.column_stack((	list(map(', '.join, self.ucf_genomes["SeqIDLists"])),
								...
								[", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in zip(TaxoOfMaxSimScore_TaxoOfMaxSimScoreRange[0], TaxoOfMaxSimScore_TaxoOfMaxSimScoreRange[1])]) for TaxoOfMaxSimScore_TaxoOfMaxSimScoreRange in zip(self.final_results["TaxoOfMaxSimScoreTable"], self.final_results["TaxoOfMaxSimScoreRangeTable"])],
								[", ".join(["%s (%s)"%(Score, Range) for Score, Range in zip(MaxSimScore_MaxSimScoreRange[0], MaxSimScore_MaxSimScoreRange[1])]) for MaxSimScore_MaxSimScoreRange in zip(np.around(self.final_results["MaxSimScoreTable"],3).astype("str"), self.final_results["MaxSimScoreRangeTable"])],
								[", ".join(["%s (%s)"%(Stat, Range) for Stat, Range in zip(PhyloStat_PhyloStatRange[0], PhyloStat_PhyloStatRange[1])]) for PhyloStat_PhyloStatRange in zip(self.final_results["PhyloStatTable"], self.final_results["PhyloStatRangeTable"])],
								[", ".join(["%s (%s)"%(Taxo, Range) for Taxo, Range in zip(TaxoAssignment_TaxoAssignmentRange[0], TaxoAssignment_TaxoAssignmentRange[1])]) for TaxoAssignment_TaxoAssignmentRange in zip(self.final_results["TaxoAssignmentTable"], self.final_results["TaxoAssignmentRangeTable"])],
								["%s (%s)"%(FinalisedTaxoAssignment_FinalisedTaxoAssignmentRange[0], FinalisedTaxoAssignment_FinalisedTaxoAssignmentRange[1]) for FinalisedTaxoAssignment_FinalisedTaxoAssignmentRange in zip(self.final_results["FinalisedTaxoAssignmentList"], self.final_results["FinalisedTaxoAssignmentRangeList"])],
			...
			np.savetxt(	fname = ClassificationResultFile,
					X = np.column_stack((	list(map(', '.join, self.ucf_genomes["SeqIDLists"])),
								self.ucf_genomes["VirusNameList"],
								list(map(', '.join, self.final_results["TaxoOfMaxSimScoreTable"])),
								list(map(', '.join, np.around(self.final_results["MaxSimScoreTable"],3).astype("str"))),
								list(map(', '.join, self.final_results["PhyloStatTable"])),
								list(map(', '.join, self.final_results["TaxoAssignmentTable"])),
		...
		pickle.dump(self.final_results, open(VariableShelveFile_UcfVirus, "wb"))
