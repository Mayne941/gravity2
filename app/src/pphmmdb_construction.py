...
class PPHMMDBConstruction:
    ...
    def get_genbank(self):
        ...
        if not os.path.isfile(self.GenomeSeqFile):
            ...
            progress_msg(
                "You didn't specify an input genbank filename that exists. Downloading sequences from Genbank"
            )
            DownloadGenBankFile(
                self.GenomeSeqFile,  # RM < TODO We should give users option to provide their own gb file if not on genbank
                self.genomes["SeqIDLists"],
                self.payload["genbank_email"],
            )
        ...
        if len(gb_missing_seqs) > 0:
            ...
            progress_msg(
                f"GRAViTy detected a mismatch in sequence numbers between input genbank and VMR files. Attempting to fix with a genbank pull..."
            )
            shell("rm data/temp.gb")
            DownloadGenBankFile(
                "data/temp.gb",
                [[i] for i in gb_missing_seqs],
                self.payload["genbank_email"],
            )
            ...
            for missing_seq in gb_missing_seqs:
                ...
                for second_gb_seq_id in CleanSecondGenbankDict.keys():
                    if second_gb_seq_id == missing_seq:
                        match = True
                        CleanGenbankDict[missing_seq] = CleanSecondGenbankDict[
                            missing_seq
                        ]
                        break
                if not match:
            ...
            if len(still_missing_seqs) > 0:
                ...
                raise_gravity_error(
                    f"There was a mismatch between the number of sequences in your VMR and the sequences you provided."
    ...
    def sequence_extraction(self, GenBankDict):
        ...
        for (
            ...
            total=self.genomes["TaxoGroupingList"].shape[0],
        ):
            for SeqID in SeqIDList:
                ...
                if allow_genbank_annotations:
                    ...
                    for Feature in GenBankFeatures:
                        if (
                            Feature.type == "CDS"
                            and "protein_id" in Feature.qualifiers
                ...
                if not ContainProtAnnotation:
                    prots, prot_ids, raw_nas = find_orfs(
                        GenBankID,
                        GenBankRecord.seq,
                        TranslTable,
                        self.payload["ProteinLength_Cutoff"],
                        taxonomy_annots=[
                            BaltimoreGroup,
                            Order,
                            Family,
                    ...
                    if len(prots) == 0:
                        raise_gravity_warning(
                            f"Sequence {SeqID} doesn't code for any ORFs!"
                        )
                    ProtList += prots
                    ProtIDList += prot_ids
        ...
        if len(ProtList) == 0:
            raise_gravity_error(
                f"No protein sequences could be extracted from input sequences. Did you input untranslated RNA sequences?"
            )

        with open(self.fnames["MashSubjectFile"], "w") as MashSubject_txt:
            SeqIO.write(
                ProtList, MashSubject_txt, "fasta"
            )  # RM < TODO Swap to pure py?
        ...
        TaxoLabelList = TaxoLabel_Constructor(
        ...
        with open(self.fnames["RefSeqFile"], "w") as f:
    ...
    def mash_analysis(self, ProtList):
        ...
        if self.payload["UseBlast"]:
            ...
            """Do Mash"""
            out = shell(
                f"mash sketch -p {self.payload['N_CPUs']} -a -i {self.fnames['MashSubjectFile']}",
                ret_output=True,
            )  # TODO PARAMETERISE MASH CALL
            error_handler_mash_sketch(
                out, "Mash (PPHMMDB Construction, mash_analysis(), initial sketch)"
            )
        ...
        with open(self.fnames["MashQueryFile"], "w") as MashQuery_txt:
            for ProtSeq in ProtList:
                MashQuery_txt.write(
                    f">{ProtSeq.name} {ProtSeq.description}\n{str(ProtSeq.seq)}\n"
                )
        # Sketch the combined FASTA file
        out = shell(
            f"mash sketch -p {self.payload['N_CPUs']} -a -i {self.fnames['MashQueryFile']}",
            ret_output=True,
        )
        error_handler_mash_sketch(
            out, "Mash (PPHMMDB Construction, mash_analysis(), combined sketch)"
        )
        # Run mash dist on the combined file
        mash_fname = f"{'/'.join(self.fnames['MashOutputFile'].split('/')[:-1])}/mashup_scores.tab"
        out = shell(
            f"mash dist -p {self.payload['N_CPUs']} -v {self.payload['Mash_p_val_cutoff']} -d {self.payload['Mash_sim_score_cutoff']} -i {self.fnames['MashQueryFile']}.msh {self.fnames['MashQueryFile']}.msh > {mash_fname}",
            ret_output=True,
        )
        error_handler_mash_dist(
            out, "Mash (PPHMMDB Construction, mash_analysis(), combined dist)"
        )
        ...
        try:
            mash_df = pd.read_csv(
                ...
                sep="\t",
                header=None,
                names=["orf", "query", "dist", "p", "hashes"],
                engine="pyarrow",
            )
        except:
            raise_gravity_warning(
                "Mash output is empty. This usually happens when an ORF hasn't been translated properly."
            )
        if mash_df.shape[0] == 0:
            raise_gravity_error(
                "Output from Mash (PPHMMDB Construction, mash_analysis()) was empty. Check Mash is installed, active in your environment and your input fastas are valid. Check pandas and pyarrow are installed (used in reading Mash output)."
            )
        else:
            ...
            if MashMatrix.shape[0] == 0:
                try:
                    ...
                    raise_gravity_error(
                        f"No Mash results were extracted from protein sequences, then failed to run BLASTp as a failover. "
                        f"Check Mash is installed and that your parameters aren't too restrictive."
                        f"Check BLAST is installed."
                    )
        np.savetxt(
            fname=self.fnames["MashSimFile"],
            X=MashMatrix,
            fmt="%s",
            delimiter="\t",
    ...
    def mcl_clustering(self, ProtIDList):
        ...
        progress_msg(
            "- Doing protein sequence clustering based on Mash bit scores, using the MCL algorithm"
        )
        out = shell(
            f"mcl {self.fnames['MashSimFile']} --abc -o {self.fnames['MashProtClusterFile']} -I {self.payload['ProtClustering_MCLInflation']}",
            ret_output=True,
        )
    ...
    def make_alignments(self, ProtList, ProtIDList):
        ...
        _, Cluster_i, Cluster_MetaDataDict = (
            LineCount(self.fnames["MashProtClusterFile"]) + 1,
            0,
            {},
        )
        with open(self.fnames["MashProtClusterFile"], "r") as MashProtCluster_txt:
            for Cluster in alive_it(MashProtCluster_txt.readlines()):
                HitList, TaxoLists, DescList, Cluster = (
                    [],
                    [],
                    [],
                    Cluster.split("\n")[0].split("\t"),
                ...
                AlnClusterFile = (
                    f"{self.fnames['ClustersDir']}/Cluster_{Cluster_i}.fasta"
                )
                with open(AlnClusterFile, "w") as UnAlnClusterTXT:
                    [
                        UnAlnClusterTXT.write(
                ...
                if len(HitList) > 1:
                    ...
                    out = shell(
                        f"mafft --thread {self.payload['N_CPUs']} --anysymbol {option} {AlnClusterFile} > {temp_aln_fname}",
                        ret_output=True,
                    )
                    error_handle_mafft(
                        out, "mafft (PPHMMDB Construction: make_alignments)"
                    )
                ...
                """Cluster annotations"""
                Cluster_MetaDataDict[Cluster_i] = {
                    "Cluster": Cluster,
                    "DescList": DescList,
                    "TaxoLists": TaxoLists,
                    "AlignmentLength": AlignIO.read(
                        temp_aln_fname, "fasta"
                    ).get_alignment_length(),
                }

                shell(
                    f"rm {AlnClusterFile} && mv {temp_aln_fname} {AlnClusterFile}",
    ...
    def pphmm_and_merge_alignments(self, Cluster_MetaDataDict):
        ...
        for Cluster_i in alive_it(range(len(Cluster_MetaDataDict))):
            ...
            HHsuite_PPHMMFile = (
                f"{self.fnames['HHsuite_PPHMMDir']}/PPHMM_{Cluster_i}.hmm"
            )
            shell(
                f"hhmake -i {AlnClusterFile} -o {HHsuite_PPHMMFile} -seq {len(Cluster_MetaDataDict[Cluster_i]['Cluster']) + 1} -name Cluster_{Cluster_i} -id 100 -M 50 -v 0",
                "PPHMMDB Construction: merge alignments, hhmake",
            )

        """Make PPHMM DB"""
        AlignmentMerging_i_round = self.rebuild_hhsuite_db(
            AlignmentMerging_i_round, fname, first=True
        )

        """Merge protein alignments"""
        while True:
            progress_msg(f"\t\t - Rebuilding HMMER PPHMM DB...")
            _ = self.Make_HMMER_PPHMM_DB(
                PphmmDb=f"{self.fnames['HMMER_PPHMMDbDir']}/HMMER_PPHMMDb_{AlignmentMerging_i_round}",
                Cluster_MetaDataDict=Cluster_MetaDataDict,
            )

            shell(
                f"find {self.fnames['HMMER_PPHMMDir']} -type f -name '*.hmm' -delete",
                "PPHMMDB Construction: merge alignments, hhsearch",
            )

            """Inter-PPHMM similarity scoring"""
            progress_msg(
                f"\t - Alignment Merging Round {AlignmentMerging_i_round + 1} - Determine PPHMM-PPHMM similarity scores (ALL-VERSUS-ALL hhsearch)"
            )
            ...
            for PPHMM_i in alive_it(range(0, N_PPHMMs)):
                HHsuite_PPHMMFile = (
                    f"{self.fnames['HHsuite_PPHMMDir']}/PPHMM_{PPHMM_i}.hmm"
                )
                ...
                shell(
                    f"hhsearch -maxmem 9.0 -i {HHsuite_PPHMMFile} -d {fname} -o {hhsearchOutFile} -e {self.payload['HHsuite_evalue_Cutoff']} -E {self.payload['HHsuite_evalue_Cutoff']} -cov {self.payload['HHsuite_SubjectCoverage_Cutoff']} -z 1 -b 1 -id 100 -global -v 0 -cpu {self.payload['N_CPUs']}",
                    "PPHMMDB Construction: merge alignments, hhsearch",
                )
                with open(hhsearchOutFile, "r") as hhsearchOut_txt:
                    ...
                    for line in Content[9:]:
                        if line == "\n":
                            ...
                            line_fil = (
                                line_fil.replace("(", " ").replace(")", " ").split()
                            )
                            try:
                                ...
                                try:
                                    ...
                                    raise_gravity_error(
                                        f"Failed to extract PPHMM data from HHsearch output (pphmmdb construction, aln merging function). This happens when HHsearch outputs malformed text files, check the output for the entry listed in this exception: {ex}"
                                    )
                            ...
                            if qcovs >= self.payload["HHsuite_QueryCoverage_Cutoff"]:
                                ...
                                if Pair in SeenPair:
                                    ...
                                    if (
                                        PPHMMSimScore
                                        > PPHMMSimScoreCondensedMat[SeenPair[Pair]][2]
                                    ):
                                    ...
                                    PPHMMSimScoreCondensedMat.append(
                                        [PPHMM_i, PPHMM_j - 1, PPHMMSimScore]
                                    )  # -1 added for zero indedxing
                                    SeenPair_i = SeenPair_i + 1

                shell(
                    f"rm {hhsearchOutFile}",
                    "PPHMMDB Construction: merge alignments, remove hhsearch file",
                )
            clean_stdout()
            ...
            if PPHMMSimScoreCondensedMat.shape[0] == 0:
                raise ValueError(
                    f"Failed on PPHMM Alignment Merging step: no alignments could be merged. Check your thresholds or disable this feature."
                )

            PPHMMSimScoreMat = np.zeros((N_PPHMMs, N_PPHMMs))
            PPHMMSimScoreMat[
                list(map(int, PPHMMSimScoreCondensedMat[:, 0])),
                list(map(int, PPHMMSimScoreCondensedMat[:, 1])),
            ] = list(map(float, PPHMMSimScoreCondensedMat[:, 2]))
            PPHMMSimScoreMat[
                list(map(int, PPHMMSimScoreCondensedMat[:, 1])),
                list(map(int, PPHMMSimScoreCondensedMat[:, 0])),
            ] = list(map(float, PPHMMSimScoreCondensedMat[:, 2]))
            PPHMMSimScoreCondensedMat = np.array(
                [
                    PPHMMSimScorePair
                    for PPHMMSimScorePair in PPHMMSimScoreCondensedMat
                    if PPHMMSimScorePair[0] < PPHMMSimScorePair[1]
                ]
            )
            PPHMMSimScoreCondensedMatFile = (
                f"{hhsearchDir}/PPHMMSimScoreCondensedMat.txt"
            )
            np.savetxt(
                fname=PPHMMSimScoreCondensedMatFile,
            ...
            shell(
                f"mcl {PPHMMSimScoreCondensedMatFile} --abc -o {PPHMMClustersFile} -I 5",  # TODO REPARAMETERISE
            ...
            with open(PPHMMClustersFile, "a") as PPHMMClusters_txt:
                PPHMMClusters_txt.write(
                    "\n".join(
                        list(
                            set(map(str, list(map(float, list(range(0, N_PPHMMs))))))
                            - set(SeenProtIDList)
                        )
                    )
                )
            ...
            if (
                N_PPHMMs_AfterMerging == N_PPHMMs
                or AlignmentMerging_i_round == self.payload["N_AlignmentMerging"]
            ):
                progress_msg(
                ...
                shell(
                    f"rm -rf {hhsearchDir}",
                    "PPHMMDB Construction: merge alignments, rm hhsearch dir",
                )
                break
            else:
                progress_msg(
            ...
            PPHMMDissimScoreMat = 1 - np.nan_to_num(
            ...
            with open(PPHMMClustersFile, "r") as PPHMMClusters_txt:
                for PPHMMCluster in alive_it(PPHMMClusters_txt.readlines()):
                    PPHMMCluster = list(
                        map(
                            int,
                            list(map(float, PPHMMCluster.split("\n")[0].split("\t"))),
                        )
                    )
                    ...
                    if len(PPHMMCluster) == 1:
                        ...
                        PPHMMDissimScoreMat_Subset = PPHMMDissimScoreMat[PPHMMCluster][
                            :, PPHMMCluster
                        ]
                        PPHMMTreeNewick = DistMat2Tree(
                            DistMat=PPHMMDissimScoreMat_Subset,
                        ...
                        while True:
                            ...
                            if not m:
                                out = shell(
                                    f"mafft --thread {self.payload['N_CPUs']} --auto --maxiterate 1000  {ClusterFile_i} > {mafft_temp_fname}",
                                    ret_output=True,
                                )
                                error_handle_mafft(
                                    out,
                                    "mafft (PPHMMDB Construction, pphmm_and_merge_alignments, terminal realignment)",
                                )
                                shell(
                                    f"rm {ClusterFile_i} && mv {mafft_temp_fname} {ClusterFile_i}",
                            ...
                            ClusterFile_i = (
                                f"{self.fnames['ClustersDir']}/Cluster_{PPHMM_i}.fasta"
                            )
                            ClusterFile_j = (
                                f"{self.fnames['ClustersDir']}/Cluster_{PPHMM_j}.fasta"
                            )
                            HHsuite_PPHMMFile_j = (
                                f"{self.fnames['HHsuite_PPHMMDir']}/PPHMM_{PPHMM_j}.hmm"
                            )

                            out = shell(
                                f"mafft --thread {self.payload['N_CPUs']} --auto --addfragments {ClusterFile_j} --reorder {ClusterFile_i} > {mafft_temp_fname}",
                                ret_output=True,
                            )
                            error_handle_mafft(
                                out,
                                "mafft (PPHMMDB Construction, pphmm_and_merge_alignments, profile-profile alignment)",
                            )
                            shell(
                                f"rm {ClusterFile_i} {ClusterFile_j} {HHsuite_PPHMMFile_j} && mv {mafft_temp_fname} {ClusterFile_i}",
                                "PPHMMDB Construction: merge alignments, rm mafft intermediates",
                            )

                            Cluster_MetaDataDict[PPHMM_i]["Cluster"] = (
                                Cluster_MetaDataDict[PPHMM_i]["Cluster"]
                                + Cluster_MetaDataDict[PPHMM_j]["Cluster"]
                            )
                            Cluster_MetaDataDict[PPHMM_i]["DescList"] = (
                                Cluster_MetaDataDict[PPHMM_i]["DescList"]
                                + Cluster_MetaDataDict[PPHMM_j]["DescList"]
                            )
                            Cluster_MetaDataDict[PPHMM_i]["TaxoLists"] = (
                                Cluster_MetaDataDict[PPHMM_i]["TaxoLists"]
                        ...
                        shell(
                            f"hhmake -i {ClusterFile_i} -o {HHsuite_PPHMMFile_i} -v 0 -seq {len(Cluster_MetaDataDict[PPHMM_i]['Cluster']) + 1} -name Cluster_{PPHMM_i} -id 100 -M 50",
                            "PPHMMDB Construction: merge alignments, hhmake",
                        )
                    AfterMergingPPHMM_i += 1

            """Rename protein alignments and their associated PPHMMs"""
            AfterMergingPPHMM_IndexList, AfterMergingPPHMM_i = (
                sorted(AfterMergingPPHMM_IndexList),
                0,
            )
            for PPHMM_i in AfterMergingPPHMM_IndexList:
                ...
                ClusterFile_j = (
                    f"{self.fnames['ClustersDir']}/Cluster_{AfterMergingPPHMM_i}.fasta"
                )
                shell(
                    f"mv {ClusterFile_i} {ClusterFile_j}",
                ...
                HHsuite_PPHMMFile_j = (
                    f"{self.fnames['HHsuite_PPHMMDir']}/PPHMM_{AfterMergingPPHMM_i}.hmm"
                )
                shell(
                    f"mv {HHsuite_PPHMMFile_i} {HHsuite_PPHMMFile_j}",
            ...
            AlignmentMerging_i_round = self.rebuild_hhsuite_db(
                AlignmentMerging_i_round, fname
            )
            shell(
                f"rm -rf {hhsearchDir}",
        ...
        """Delete dir"""
        shell(
            f"rm -rf {self.fnames['HHsuiteDir']}",
    ...
    def rebuild_hhsuite_db(self, AlignmentMerging_i_round, fname, first=False):
        ...
        shell(
            f"rm {self.fnames['HHsuite_PPHMMDB']}_hhm.ffdata {self.fnames['HHsuite_PPHMMDB']}_hhm.ffindex",
            "PPHMMDB Contruction: rebuild hhsite db, remove existing db",
        )
        shell(
            f"ffindex_build -s {fname}_msa.ffdata {fname}_msa.index {self.fnames['ClustersDir']}"
        )
        shell(
            f"ffindex_apply {fname}_msa.ffdata {fname}_msa.index -i {fname}_a3m.ffindex -d {fname}_a3m.ffdata -- hhconsensus -M 50 -maxres 65535 -i stdin -oa3m stdout -v 0"
        )
        shell(f"rm {fname}_msa.ffdata {fname}_msa.index")
        shell(
            f"ffindex_apply {fname}_a3m.ffdata {fname}_a3m.ffindex -i {fname}_hhm.ffindex -d {fname}_hhm.ffdata -- hhmake -i stdin -o stdout -v 0"
        )
        shell(f"cstranslate -f -x 0.3 -c 4 -I a3m -i {fname}_a3m -o {fname}_cs219")
        shell(
            f"sort -k3 -n -r {fname}_cs219.ffindex | cut -f1 > sorting.dat"
        )  # Parameterise sorting file
        shell(
            f"ffindex_order sorting.dat {fname}_hhm.ff{{data,index}} {fname}_hhm_ordered.ffdata {fname}_hhm_ordered.ffindex"
        )
        shell(f"mv {fname}_hhm_ordered.ffindex {fname}_hhm.ffindex")
        shell(f"mv {fname}_hhm_ordered.ffdata {fname}_hhm.ffdata")
        shell(
            f"ffindex_order sorting.dat {fname}_a3m.ffdata {fname}_a3m.ffindex {fname}_a3m_ordered.ffdata {fname}_a3m_ordered.ffindex"
        )
    ...
    def Make_HMMER_PPHMM_DB(self, PphmmDb, Cluster_MetaDataDict):
        ...
        for PPHMM_i in alive_it(range(N_PPHMMs)):
            ...
            ClusterSizeByTaxoGroupingList.append(
                ", ".join(
                    [
                        f"{TaxoGrouping}: {N_ProtSeqsInTheTaxoGroup}"
                        for TaxoGrouping, N_ProtSeqsInTheTaxoGroup in sorted(
                            list(Counter(list(zip(*TaxoLists))[-1]).items()),
            ...
            ClusterSizeByProtList.append(
                ", ".join(
                    [
                        f"{Desc}: {N_ProtSeqsWithDesc}"
                        for Desc, N_ProtSeqsWithDesc in sorted(
                            list(Counter(DescList).items()),
                            key=operator.itemgetter(1),
                            reverse=True,
                        )
                    ]
            ...
            for ClusterTaxo_UniqueTaxoLabel in [
                ClusterTaxo_UniqueBaltimoreGroup,
                ClusterTaxo_UniqueOrder,
                ClusterTaxo_UniqueFamily,
                ClusterTaxo_UniqueSubFam,
                ...
                if len(ClusterTaxo_UniqueTaxoLabel) == 1:
                    if ClusterTaxo_UniqueTaxoLabel[0] not in [
                        "",
                        "Unassigned",
                        "unassigned",
                    ]:
            ...
            for ClusterDesc in ClusterDescCount:
                ...
                if (
                    ("Hypothetical" not in ClusterDesc)
                    and ("hypothetical" not in ClusterDesc)
                    and ("~" not in ClusterDesc)
                ):
                    break
            ...
            out = shell(
                f"hmmbuild --amino {HMMER_PPHMMFile} {AlnClusterFile}", ret_output=True
            )
            error_handler_hmmbuild(
                out, "hmmbuild (PPHMMDB Construction: Make_HMMER_PPHMM_DB)"
            )
        ...
        shell(
            f"find {self.fnames['HMMER_PPHMMDir']} -name '*.hmm' -exec cat {{}} \; > {PphmmDb}",
            "PPHMMDB Construction: make HMMER PPHMMDB, find db",
        )
        shell(
            f"hmmpress -f {PphmmDb}",
            "PPHMMDB Construction: make HMMER PPHMMDB, HMMPress",
        )
        """Make a PPHMMDBSummary file"""
        ClusterIDList = [f"Cluster_{Cluster_i}" for Cluster_i in range(N_PPHMMs)]
        np.savetxt(
            fname=f"{PphmmDb}_Summary.txt",
            X=np.column_stack(
                (
                    ClusterIDList,
                    ClusterDescList,
        ...
        pphhmmdb_construction_out["ClusterDescList"] = np.array(ClusterDescList)
        pphhmmdb_construction_out["ClusterSizeList"] = np.array(ClusterSizeList)
    ...
    def main(self):
        ...
        Cluster_MetaDataDict = self.make_alignments(ProtList, ProtIDList)
